...
public class MainController {
	...
	@GetMapping("/emplist")
	public String openEmpListPage(@RequestParam(name = "redirectAttr_success", required = false) String success,
	...
	public String registerEmployeeForm(@Valid @ModelAttribute("modelEmpAttr") Employee emp, BindingResult br,
			Model model) {
		if (!br.hasErrors()) {
	...
	public String updateEmployeeForm(@Valid @ModelAttribute("modelEmpAttr") Employee emp, BindingResult br,
			Model model) {
		if (!br.hasErrors()) {
	...
	@GetMapping("/forcast")
	public String OpenForcastform(Model model,
	...
	@PostMapping("/forcastGraph")
	public String openGraph(@RequestParam("startdate") String date, @RequestParam("coursename") String coursename,
			Model model) {
        ...
        LocalDate date11 = LocalDate.parse(date, originalFormatter);

        // Format the LocalDate object to the target pattern
        String formattedDate = date11.format(targetFormatter);
		
		
		
		System.out.println("reversedStr =>"+formattedDate);
	

		List<Object[]> list = productService.getPurchasedCourseCountService(formattedDate, coursename);
